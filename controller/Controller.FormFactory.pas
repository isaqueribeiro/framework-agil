...
uses
  ...
  type
    ...
    public
      ...
      function CreateForm(const AOnwer : TComponent; const aFormName: String): TFormDefaultUI; overload;
...
function TFormFactory.CreateForm(const AOnwer: TComponent;
  const aFormName: string): TFormDefaultUI;
var
  i : Integer;
begin
  i := FList.IndexOf(aFormName);
  Assert(i <> -1, 'Formulário ' + aFormName + ' não existe');
  Result := TComponentClass(FList.Objects[i]).Create(AOnwer) as TFormDefaultUI;
end;
